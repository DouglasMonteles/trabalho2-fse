...
int32_t init_bme280() {
    if((file_descriptor = open(I2C_DIR_PATH, O_RDWR)) < 0) return BME280_RESPONSE_FAIL;
    if(ioctl(file_descriptor, I2C_SLAVE, BME280_I2C_ADDR_PRIM) < 0) return BME280_RESPONSE_FAIL;
    ...
    if(result != BME280_OK) return BME280_RESPONSE_FAIL;
    ...
    return BME280_RESPONSE_SUCCESS;
}
...
int8_t user_i2c_write(uint8_t reg_addr, const uint8_t *data, uint32_t len, __attribute__((unused)) void *intf_ptr){
    ...
    if(write(file_descriptor, buffer, len+1) < (uint16_t)len) return BME280_RESPONSE_FAIL;
...
//     double temp_ext;
//     int result = get_temperature(&temp_ext);
